...
def dict_merge_in_place_list_replace(left: Mergeable, right: Mergeable):
    """Merges right into left in place and returns merged result
    ...
    try:
        if left is None or isinstance(left, (str, int, float, bool)):
            ...
            if isinstance(right, dict):
                for key in right:
                    if key in left:
                        left[key] = dict_merge_in_place_list_replace(left[key], right[key])
                    else:
                        left[key] = right[key]
            else:
